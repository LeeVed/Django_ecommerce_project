...
class BlogPost(models.Model):
    ...
    title = models.CharField(
        max_length=200, verbose_name="Заголовок", help_text="Введите заголовок статьи (макс. 200 символов)"
    )

    # Делаем красивые url
    slug = models.SlugField(
        max_length=200, unique=True, verbose_name="Слаг", help_text="Уникальная часть URL (заполняется автоматически)"
    )
    ...
    created_at = models.DateTimeField(
        default=timezone.now, verbose_name="Дата создания", help_text="Дата и время создания записи"
    )

    # Признак публикации
    is_published = models.BooleanField(
        default=False, verbose_name="Опубликовано", help_text="Отметьте для публикации статьи"
    )
